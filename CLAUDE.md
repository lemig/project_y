...
2. **Harness:** Deep Agents (LangChain, Apache 2.0). Wrapped behind an internal `AgentHarness` adapter (`src/agent/harness.py`) exposing a stable interface (`planner_run`, `spawn_subagent`, `load_skill`, `checkpoint`, `resume`). Pinned exact version. Golden-run replay tests gate dep upgrades.
